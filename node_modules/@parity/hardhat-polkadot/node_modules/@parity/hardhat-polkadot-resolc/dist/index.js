...
(0, config_1.extendConfig)((config, userConfig) => {
    ...
    const defaultConfig = isBinary ? constants_1.defaultBinaryResolcConfig : constants_1.defaultNpmResolcConfig;
    const customConfig = userConfig?.resolc || {};
    const optimizer = Object.assign({}, defaultConfig.settings?.optimizer, customConfig.settings?.optimizer);
    config.resolc = {
        ...defaultConfig,
        ...customConfig,
        settings: {
...
(0, config_1.extendEnvironment)((hre) => {
    ...
    if ((hre.config.solidity.compilers.length > 1 && hre.config.resolc.compilerSource === "npm") ||
        (Object.entries(hre.config.solidity.overrides).length > 1 &&
            hre.config.resolc.compilerSource === "npm")) {
        throw new errors_2.ResolcPluginError(`Multiple solidity versions are not available when using npm as the compiler.`);
    }
...
(0, config_1.task)(task_names_1.TASK_COMPILE).setAction(async (
// eslint-disable-next-line @typescript-eslint/no-explicit-any
compilationArgs, _hre, runSuper) => {
    await runSuper(compilationArgs);
});
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_GET_SOURCE_NAMES, async (args, hre, runSuper) => {
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_GET_ARTIFACT_FROM_COMPILATION_OUTPUT, async ({ sourceName, contractName, contractOutput, }, hre) => {
    if (!hre.network.polkavm) {
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_RUN_SOLC, async (args, hre, runSuper) => {
    if (!hre.network.polkavm) {
        return await runSuper(args);
    }
    return await hre.run(constants_1.TASK_COMPILE_SOLIDITY_RUN_RESOLC, {
        input: args.input,
        solcPath: args.solcPath,
        solcVersion: args.solcVersion,
    });
});
(0, config_1.subtask)(constants_1.TASK_COMPILE_SOLIDITY_RUN_RESOLC, async (args, hre) => {
    const config = {
        ...hre.config.resolc,
        settings: {
    ...
    return await (0, compile_1.compile)(config, args.input);
});
(0, config_1.subtask)(constants_1.TASK_COMPILE_SOLIDITY_COMPILE_RESOLC, async (args, hre) => {
    const solcBuild = await hre.run(task_names_1.TASK_COMPILE_SOLIDITY_GET_SOLC_BUILD, {
        quiet: args.quiet,
        solcVersion: args.solcVersion,
        compilationJob: args.compilationJob,
    });
    const resolcBuild = await hre.run(constants_1.TASK_COMPILE_SOLIDITY_GET_RESOLC_BUILD, {
        quiet: args.quiet,
    ...
    const output = await hre.run(constants_1.TASK_COMPILE_SOLIDITY_RUN_RESOLC, {
        input: args.input,
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_COMPILE_SOLC, async (args, hre, runSuper) => {
    if (!hre.network.polkavm) {
        return await runSuper(args);
    }
    return await hre.run(constants_1.TASK_COMPILE_SOLIDITY_COMPILE_RESOLC, args);
});
(0, config_1.subtask)(constants_1.TASK_COMPILE_SOLIDITY_LOG_DOWNLOAD_RESOLC_COMPILER_START).setAction(async ({ isCompilerDownloaded, resolcVersion, }) => {
...
(0, config_1.subtask)(constants_1.TASK_COMPILE_SOLIDITY_GET_RESOLC_BUILD).setAction(async ({ quiet, resolcVersion, }, { run }) => {
    const compilersCache = await (0, global_dir_1.getCompilersDir)();
    const compilerPlatform = downloader_2.ResolcCompilerDownloader.getCompilerPlatform();
    const downloader = downloader_2.ResolcCompilerDownloader.getConcurrencySafeDownloader(compilerPlatform, compilersCache);
    await downloader.downloadCompiler(resolcVersion, async (isCompilerDownloaded) => {
        await run(constants_1.TASK_COMPILE_SOLIDITY_LOG_DOWNLOAD_RESOLC_COMPILER_START, {
    ...
    logDebug("Native resolc binary doesn't work, using wasm instead. Try running npx hardhat clean --global");
    const wasmDownloader = downloader_2.ResolcCompilerDownloader.getConcurrencySafeDownloader(downloader_1.CompilerPlatform.WASM, compilersCache);
    await wasmDownloader.downloadCompiler(resolcVersion, async (isCompilerDownloaded) => {
    ...
    (0, errors_1.assertHardhatInvariant)(wasmCompiler !== undefined, `WASM build of resolc ${resolcVersion} isn't working`);
    return wasmCompiler;
});
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_LOG_COMPILATION_RESULT, async ({ compilationJobs }, _hre, _runSuper) => {
    let count = 0;
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_LOG_RUN_COMPILER_START).setAction(async ({ compilationJob, }) => {
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_EMIT_ARTIFACTS).setAction(async ({ compilationJob, input, output, solcBuild, }, { artifacts, run, network }, runSuper) => {
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_GET_COMPILER_INPUT, async (taskArgs, hre, runSuper) => {
...
(0, config_1.subtask)(task_names_1.TASK_COMPILE_REMOVE_OBSOLETE_ARTIFACTS, async (taskArgs, hre, runSuper) => {
    if (!hre.network.polkavm) {
