"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const node_test_1 = require("node:test");
const node_fs_1 = require("node:fs");
const node_assert_1 = __importDefault(require("node:assert"));
const _1 = require(".");
const node_path_1 = require("node:path");
const compileOptions = [{}];
if ((0, node_fs_1.existsSync)('../../target/release/resolc')) {
    compileOptions.push({ bin: '../../target/release/resolc' });
}
for (const options of compileOptions) {
    (0, node_test_1.test)(`check Ok output with option ${JSON.stringify(options)}`, async () => {
        const contract = 'fixtures/token.sol';
        const sources = {
            [contract]: {
                content: (0, node_fs_1.readFileSync)('fixtures/storage.sol', 'utf8'),
            },
        };
        const out = await (0, _1.compile)(sources, options);
        (0, node_assert_1.default)(out.contracts[contract].Storage.abi != null);
        (0, node_assert_1.default)(out.contracts[contract].Storage.evm.bytecode != null);
    });
}
(0, node_test_1.test)('check Err output', async () => {
    const sources = {
        bad: {
            content: (0, node_fs_1.readFileSync)('fixtures/storage_bad.sol', 'utf8'),
        },
    };
    const out = await (0, _1.compile)(sources);
    (0, node_assert_1.default)(out?.errors?.[0].message.includes('SPDX license identifier not provided in source file'));
    (0, node_assert_1.default)(out?.errors?.[1].message.includes('Source file does not specify required compiler version'));
});
(0, node_test_1.test)('check Err from stderr', async () => {
    const sources = {
        bad: {
            content: (0, node_fs_1.readFileSync)('fixtures/bad_pragma.sol', 'utf8'),
        },
    };
    try {
        await (0, _1.compile)(sources);
        (0, node_assert_1.default)(false, 'Expected error');
    }
    catch (error) {
        (0, node_assert_1.default)(String(error).includes('Source file requires different compiler version'));
    }
});
(0, node_test_1.test)('resolve import', () => {
    const cases = [
        // local
        {
            file: './fixtures/storage.sol',
            expected: (0, node_path_1.resolve)('fixtures/storage.sol'),
        },
        // package with exports
        {
            file: '@redstone-finance/evm-connector/contracts/data-services/PrimaryProdDataServiceConsumerBase.sol',
            expected: (0, node_path_1.resolve)(__dirname, '../../..', 'node_modules/@redstone-finance/evm-connector/contracts/data-services/PrimaryProdDataServiceConsumerBase.sol'),
        },
        // scopped module with version
        {
            file: '@openzeppelin/contracts@5.1.0/token/ERC20/ERC20.sol',
            expected: require.resolve('@openzeppelin/contracts/token/ERC20/ERC20.sol'),
        },
        // scopped module without version
        {
            file: '@openzeppelin/contracts/token/ERC20/ERC20.sol',
            expected: require.resolve('@openzeppelin/contracts/token/ERC20/ERC20.sol'),
        },
        // scopped module with wrong version
        {
            file: '@openzeppelin/contracts@4.8.3/token/ERC20/ERC20.sol',
            expected: `Error: Version mismatch: Specified @openzeppelin/contracts@4.8.3, but installed version is 5.1.0`,
        },
        // module without version
        {
            file: '@openzeppelin/contracts/package.json',
            expected: require.resolve('@openzeppelin/contracts/package.json'),
        },
        // scopped module with version
        {
            file: '@openzeppelin/contracts@5.1.0/package.json',
            expected: require.resolve('@openzeppelin/contracts/package.json'),
        },
    ];
    for (const { file, expected } of cases) {
        let resolved;
        try {
            resolved = (0, _1.tryResolveImport)(file);
        }
        catch (error) {
            (0, node_assert_1.default)(String(error) == expected, `\nExpected:\n${expected}\nGot:\n${String(error)}\n`);
        }
        if (resolved) {
            (0, node_assert_1.default)(resolved === expected, `\nExpected:\n${expected}\nGot:\n${resolved}`);
        }
    }
});
//# sourceMappingURL=index.test.js.map