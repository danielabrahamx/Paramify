...
async function compile(sources, option = {}) {
    const { optimizer = {
        ...
        enabled: true,
        runs: 200,
    }, bin = process.env.RESOLC_BIN, } = option;
 ...
 **/
function resolvePkgRoot(basePackage) {
    const packageRoot = (0, resolve_pkg_1.default)(basePackage);
    if (packageRoot) {
        return packageRoot;
    }
    try {
...
function tryResolveImport(importPath) {
    ...
    if (specifiedVersion) {
        const installedVersion = JSON.parse((0, fs_1.readFileSync)(path_1.default.join(packageRoot, 'package.json'), 'utf-8')).version;
