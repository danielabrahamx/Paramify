...
async function main() {
    ...
    program
        ...
        .option('--diff-stats', 'Print statistics about Resolc vs Solc compilation.')
    ...
    if (options.diffStats) {
        ...
        try {
            (0, child_process_1.execSync)(`npx --yes solc@latest ${args.slice(2).join(' ')}`);
        }
        catch (err) {
    ...
    for (const fileName in output.contracts) {
        for (const contractName in output.contracts[fileName]) {
            ...
            if (options.bin &&
                ...
                const pvmData = Buffer.from(output.contracts[fileName][contractName].evm.bytecode.object, 'hex');
    ...
    if (options.diffStats && contractStats.length > 0) {
        console.table(contractStats.sort((a, b) => b['resolc (kB)'] - a['resolc (kB)']));
    }
